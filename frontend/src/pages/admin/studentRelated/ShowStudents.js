...
const ShowStudents = () => {
    ...
    useEffect(() => {
        if (studentsList && studentsList.length > 0) {
            ...
            const timer = setTimeout(() => {
                ...
                const steps = 60;
                const increment = studentsList.length / steps;
                const stepDuration = duration / steps;
                
                let current = 0;
                const interval = setInterval(() => {
                    current += increment;
                    if (current >= studentsList.length) {
    ...
    const actions = [
        {
            icon: <PersonAddAlt1Icon sx={{ color: '#00E5FF' }} />, name: 'Add New Student',
            action: () => navigate("/Admin/addstudents")
        },
        {
            icon: <DeleteIcon sx={{ color: '#FF3D00' }} />, name: 'Delete All Students',
            action: () => deleteHandler(currentUser._id, "Students")
        },
    ];

    // Calculate statistics
    const uniqueClasses = new Set(studentsList?.map(student => student.sclassName.sclassName)).size || 0;
    const studentsWithAttendance = studentsList?.filter(student => student.attendance && student.attendance.length > 0).length || 0;
